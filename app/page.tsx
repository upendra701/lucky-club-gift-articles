...
export default function Home() {
  ...
  return <main>
    ...
    <section className="categories" id="gifts"><div className="heading-center"><span className="gold-rule" /><p className="eyebrow">A little something for everyone</p><h2>Find the Perfect Gift</h2><span className="gold-rule" /></div><div className="category-viewport"><div className="category-scroll"><div className="category-track">{categories.map((category, index) => <Link className={`category-card category-${(index % 7) + 1}`} href={`/products?category=${encodeURIComponent(category.slug)}`} key={category.id}><div className="category-art" style={category.image ? { backgroundImage: `url("${category.image}")` } : undefined}><span>{String(index + 1).padStart(2, "0")}</span></div><strong>{category.name}</strong><Arrow /></Link>)}</div></div></div></section>
