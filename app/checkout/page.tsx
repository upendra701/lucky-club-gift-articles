...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ product?: string; quantity?: string }> }) {
  ...
  return <main className="checkout-page"><header className="catalog-header"><Link className="catalog-brand" href="/">Lucky Club <small>Gift Articles</small></Link><nav><Link href={`/products/${product.slug}`}>Back to gift</Link></nav></header><div className="checkout-intro"><p className="eyebrow">A thoughtful next step</p><h1>Confirm your <em>customisation.</em></h1><p>Your details will be saved with this guest order. Payment will be added in the next step of the Lucky Club journey.</p></div><CheckoutForm product={{ slug: product.slug, name: product.name, price: product.price.toString(), shippingCharge: product.shippingCharge.toString(), imageUrl: product.images[0]?.url ?? null, imageAlt: product.images[0]?.alt ?? null }} quantity={quantity} /></main>;
}
