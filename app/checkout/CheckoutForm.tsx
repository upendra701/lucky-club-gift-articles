...
type CheckoutProduct = { slug: string; name: string; price: string; shippingCharge: string; imageUrl: string | null; imageAlt: string | null };
...
export function CheckoutForm({ product, quantity: initialQuantity }: { product: CheckoutProduct; quantity: number }) {
  ...
  const shipping = Number(product.shippingCharge).toFixed(2);
  ...
  return <form className="checkout-form" action={action}>
    ...
    <section className="checkout-section checkout-summary"><p className="eyebrow">Your gift</p><div className="checkout-product"><div className="checkout-product-image">{product.imageUrl ? <img src={product.imageUrl} alt={product.imageAlt || product.name} /> : <span>LC</span>}</div><div><h2>{product.name}</h2><p>₹{product.price} each</p></div><label>Qty<select value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} aria-label="Quantity">{Array.from({ length: 20 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label></div><div className="checkout-totals"><span>Subtotal <strong>₹{subtotal}</strong></span><span>Shipping <strong>₹{shipping}</strong></span><span className="checkout-total">Total <strong>₹{total}</strong></span></div><p className="checkout-help">Shipping is charged once per order, regardless of quantity.</p></section>
