...
function productValues(formData: FormData) {
  ...
  const comparePrice = decimalText(
  ...
  return {
    ...
    customizationEnabled: booleanField(
      formData.get("customizationEnabled"),
    ),
    customizationInstructions: optionalText(
      formData.get("customizationInstructions"),
    ),
...
export async function saveProduct(formData: FormData) {
  ...
  // Open the newly created product so images can be uploaded
  // through the existing ImageManager.
  redirect(`/admin/products/${product.id}`);
}

async function removeStorageObjects(
  images: { storageBucket: string; storagePath: string }[],
) {
  if (!images.length) {
    return;
  }
  ...
  for (const image of images) {
    grouped.set(image.storageBucket, [
      ...(grouped.get(image.storageBucket) ?? []),
      image.storagePath,
    ]);
  }
  ...
  for (const [bucket, paths] of grouped) {
    ...
    if (error) {
      throw new Error(
        `Could not remove image files: ${error.message}`,
      );
    }
  }
}
...
export async function deleteProductImage(formData: FormData) {
  ...
  const { error } = await createAdminStorageClient()
    .storage
  ...
  if (error) {
    throw new Error(
      `Could not remove image file: ${error.message}`,
    );
  }
...
export async function replaceProductImage(formData: FormData) {
  ...
  const storage =
    createAdminStorageClient().storage;
  ...
  if (uploadError) {
    throw new Error(
      `Could not upload replacement: ${uploadError.message}`,
    );
  }
  ...
  if (removeError) {
    throw new Error(
      `Image updated, but the old Storage file could not be removed: ${removeError.message}`,
    );
  }
