...
function productValues(formData: FormData) {
  ...
  const shippingCharge = decimalText(formData.get("shippingCharge"), "Shipping charge");

  if (!price) throw new Error("Price is required.");
  if (!shippingCharge) throw new Error("Shipping charge is required.");
  if (Number(shippingCharge) < 0) throw new Error("Shipping charge cannot be negative.");
...
export async function updateProductImage(formData: FormData) {
  ...
  if (!image) throw new Error("Image not found.");
  await prisma.productImage.update({ where: { id: imageId }, data: { alt: optionalText(formData.get("alt")), sortOrder: Number(formData.get("sortOrder")) || 0, isPrimary: formData.get("isPrimary") === "true" } });
