...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${product.name} | Personalized Gift`,
    description: product.description || `Shop ${product.name}, a thoughtful personalized gift from Lucky Club Gift Articles. Customize your gift through WhatsApp.`,
    alternates: { canonical: `/products/${product.slug}` },
    openGraph: {
      type: "website",
      title: `${product.name} | Lucky Club Gift Articles`,
      description: product.description || `Discover ${product.name} from Lucky Club Gift Articles.`,
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productSchema = {
    ...
    "@type": "Product",
    name: product.name,
    description: product.description || `Personalized ${product.name} from Lucky Club Gift Articles.`,
    url: `${SITE_URL}/products/${product.slug}`,
    image: product.images.map((image) => image.url),
    category: product.category.name,
    brand: { "@type": "Brand", name: "Lucky Club Gift Articles" },
    offers: {
      "@type": "Offer",
      url: `${SITE_URL}/products/${product.slug}`,
      priceCurrency: "INR",
